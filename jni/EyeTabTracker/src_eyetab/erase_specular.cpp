...
void erase_specular(const Mat& eye_bgr) {
	...
	int max_spec_contour_area = (eye_bgr.size().width + eye_bgr.size().height)
			/ 2;
	...
	double thresh = (m + M) * 0.4; //0.4 is much nearer to the results of python
	...
	findContours(eye_thresh, all_contours, CV_RETR_LIST,
